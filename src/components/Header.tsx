...
const Header = () => {
  ...
  const isHeaderOnTop = useMemo(() => {
    return (location.pathname === '/' && userPosition === 0) ? cl.mainTop : ''
    ...
    <header className={`${cl.header} ${isHeaderVisible ? cl.visible : ''} ${isHeaderOnTop}`}>
      ...
      <div className={cl.content_wrapper}>
        <nav>
          ...
          <Link to='/'><li>Магазин</li></Link>
          <Link to='/'><li>Моё</li></Link>
          <Link to='/'><li>Телеканалы</li></Link>
          <Link to='/'><li>Спорт</li></Link>
        </nav>
        <svg width="2.4rem" height="2.4rem" viewBox="0 0 24 24" version="1.1" xmlns="http://www.w3.org/2000/svg">
          <path fillRule="evenodd" d="M16.6 10.5a6.1 6.1 0 1 1-12.2 0 6.1 6.1 0 0 1 12.2 0Zm-.998 6.8a8.5 8.5 0 1 1 1.697-1.697l5.55 5.548-1.698 1.698-5.549-5.55Z" fill="#fff"></path>
        </svg>
      </div>
